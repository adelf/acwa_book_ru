...
Пример практически скопирован из документации, и он показывает всю мощь и элегантность Laravel. Это хорошо, фреймворк и должен быть таким, предоставляющим хорошие инструменты для решения стандартных задач.
Однако, 

потом это будет как хороший дом, к которому пристраивают и пристраивают всякое.
...
```
Здесь был нарушен Принцип единственной ответственности (Single Responsibility Principle). Класс **ImageUploader** теперь занимается далеко не только загрузкой изображений, но и кучей разных других дел. У него присутствуют и другие проблемы, но мы поговорим о них позже. 
...
Когда я отрефакторил этот метод созданием разных методов **create** and **update** и выносом общей логики в нужные методы и классы, код стал в разы проще для чтения.
...
IsBanned=true
```
Хорошо. Можно предположить, что **IsBanned** это поле класса User, но когда пользователь реально будет забанен, то приложение должно, например, послать письмо этому пользователю.
Чтобы реализовать это требование должны быть реализованы не самые простые проверки «старых»(до изменения сущности данным запросом) и «новых» значений. 
...
Оптимистичная блокировка помогла бы с этой проблемой, но она не очень популярна в Laravel проектах (я нашел лишь пару пакетов для ее реализации, но все они имеют меньше 50 звезд на Github).
...
Эта книга о том, как обойти их и про хорошие привычки, позволяющие создавать легко расширяемый и поддерживаемый код.
Давайте начнем!
