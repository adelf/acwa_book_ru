...
Код все-еще выглядит неплохо, но количество дублированного кода растет.
...
**ImageUploader::upload** метод возвращает **false** если загрузка была неуспешной, например при ошибке облачного хранилища или неприемлемом контенте.
Или URL-адрес картинки при удачной загрузке.
...
Методы контроллеры стали проще, поскольку логика загрузки аватаров вынесена в другой класс. Отлично!
...
Их реализацией могут заниматься другой разработчики, которые не понимают почему этот код был написан таким образом.
...
Всего 4 глагола: **GET**(прочитать), **POST**(создать), **PUT/PATCH**(изменить) and **DELETE**(удалить). 
...
```
Как видите, любой нестандартный глагол и RESTful подход становится весьма неоднозначным.
...
Другое пример: изменение пароля.
...
Я написал простой класс для работы с ключами кеширования для одного из проектов.
Ключи кеширования нужны как минимум в двух местах: при чтении из кеша и при удалении значений оттуда до срока.
Очевидное решение: 
...
```
Laravel, начиная с какой-то версии предлагает использовать «implicit route binding». Этот код работает также как предыдущий:
