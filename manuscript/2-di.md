...
Каждое использование ключевого слова protected создаёт неявную связь между классам предка и наследника.
...
Но мне все еще не нравится параметры метода **upload**.
...
Для текущего примера решение выглядит черезчур громоздким.
...
Вместо того, чтобы комбинировать класс из трейтов, намного лучше разбить ответственности класса и использовать этот класс как комбинация этих ответственностей.
...
Некоторые трейты просто абсолютно бесполезны.
...
В случае, когда этот класс заходят перенести в другой проект или написать тесты для него, разработчикам не придется полность сканировать его код и искать эти глобальные вызовы функций или фасадов.
...
Надо лечить боезнь, а не симптомы, поэтому лучше найти причины, заставляющие нас использовать трейты и постараться их исправить.
