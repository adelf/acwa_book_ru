...
```
Таким образом, мы берем на себя обязательство лично просмотреть метод `create()` – нам же надо как-то понять, какие данные массива требуется передать в него, верно?
Но беда не приходит одна! – Ко всему прочему, мы здесь работаем с *примитивами*, в частности – строки в качестве ключей. Соответственно, элементарная опечатка способна поломать, либо магически изменить (что еще хуже поломки!) весь последующий код.
И последний гвоздь в крышку гроба такого подхода – потеря контроля над входящими типами данных. Формируя нашу дату, мы опять берем на себя обязательство проверить метод `create()`, но уже на предмет входящих *типов данных*!
Такой подход удобен и практичен в случае HTML-форм и небольших web-сайтов, но он безобразен с точки зрения надежности масштабируемых приложений. Представьте себе огромный проект, где по его участкам раскиданы вот такие конструкторы данных. И представьте теперь вероятность ошибки, с каждым новым изменением уже существующих данных.
Очевидно, что использовать массив со строками – не лучшая затея. Мягко скажем. Но за неимением иного – приходится. Однако выход есть. Причем с использованием ООП-подхода, – то есть мы перешагнем от примитивов к объектам, да еще и получая полный контроль над типами! И выход этот – **Data Transfer Object** (DTO).
Шаблон DTO предлагает просто создавать объекты с нужными типизированными полями:
