...
Сервисный класс получается чистую DTO и выполняет действие.
Правда, метод контроллера теперь довольно большой. Конструктор DTO класса может быть весьма длинным.
Можно попробовать вынесли логику работы с данными запроса оттуда.
...
```
Если какой-либо класс просит класс, отнаследованный от **FormRequest** как зависимость, Laravel создаёт его и выполняет валидацию автоматически. 
...
Авторизация часто требует контекста, например какой юзер, с какая сущностью, какое действие.
...
Всем нужна база данных, но некоторые хотят посылать письма, другие - залить файл в хранилище, третьи - дёрнуть какое-нибудь API.
