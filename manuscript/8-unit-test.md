...
A> 100% покрытие тестами должно быть следствие, а не целью.
...
Security testing для проверки прилоэений на различные уязвимости.
...
**Функцмиональное тестирование** проверяет приложение на соответсвие функциональным требованиям.
...
Он знает слишком многое про то, как работает прилоэжение, как оно хранит данные и какие таблицы для этого использует.
...
Я всегда гооврю Laravel-фасады, чтобы не путаться со структурным шаблоном **Фасад**, который про другое.
...
Соответсвенно, база данных должна быть такая же как и на сервере продакшена.
...
Это происходит потмоу, что здесь используется специальный базовый класс для тестов, предоставляемый Laravel, который полностью настраивает рабочее окружение.
...
Да, они будут работать корректно в реальном пирложении, но не в unit-тестах.
...
* Каждоая добавленная зависимость заставить переписывать все unit-тесты.
...
Он содержит основную логику (проверку количества опций ответа и создание сущности с опциями), а аткже логику приложения (транзакции базы данных, генерация событий, проверку на мат в одном из прошлых вариантов и т.д.).
...
Он может описать главные сценарии работы с прилоэение и после каждого обновления проходить по этим сценариям, проверяя, что приложение работает как требуется (это называется регрессионное тестирование). 
...
Unit-тесты же для контроллеров или Слоя Пирложения, как мы уже убедились ранее, писать весьма неприятно, а поддерживать тем более.
